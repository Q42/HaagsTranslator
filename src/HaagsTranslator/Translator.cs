...
namespace HaagsTranslator
{
  ...
  public static class Translator
  {
    ...
    static readonly string[][] TranslationReplacements = {
      ...
      new []{ "Ui", "Ùi" },
      new []{ "(?<![ideopfv])ert\\b", "egt" }, // 'gert', geen 'viert', 'expert', 'levert', 'verandert'
      ...
      new []{ "(?<![ bgio])eden", "eide" }, // 'hedendaagse', geen 'bedenken', 'woedende'
      ...
      new []{ "(?<![i])(g|b)er(?![eoiuaâèr])", "$1âh"}, // 'tijgermug', 'ubergezellige', moet na '-erg', geen 'beraden', 'vertegenwoordiger', 'berg'
      new []{ "epers\\b", "epâhs"}, // 'developers'
      new []{ "(P|p)ers(?![l])", "$1egs"}, // 'pers', geen 'superslimme'
      new []{ "(K|k)erst", "$1egs"}, // 'kerstfeest'
      new []{ "(?<![en])(t|V|v)ers(?![clt])", "$1egs"}, // 'vers', 'personeel', 'versie', 'diverse', geen 'gevers', 'verscheen', 'eerste', 'sprinters'
      ...
      new []{ "\\b(T|t)ers", "$1es"}, // 'terschelling'
      ...
      new []{ "(H|h|n|T|t)ele(?![u])", "$1eile"}, // 'gehele', 'hele', 'originele', 'televisie', geen 'teleurgesteld'
      ...
      new []{ "lf(?![e])", "lluf"}, // 'zelfde', geen 'wisselfestein'
      new []{ "ll([ ,.])", "l$1" }, // 'till'
      ...
      new []{ "(T|t)one(e|i)", "$1aunei" }, // 'toneel', geen 'oneetbaar'
      ...
      new []{ "(?<![gz ])o(b|d|g|k|l|m|n|p|s|t|v)(i|e|o|au)", "au$1$2"}, // 'komen', 'grote', 'over', 'olie', 'notie', geen 'gokje', 'foto', 'doneren', 'zone', 'oneetbaar'
      new []{ "\\bo(l|p)(i|e)", "au$1$2"}, // 'olie', 'operatie'
      new []{ "O(b|d|g|k|l|m|p|s|t|v)(i|e)", "Au$1$2"}, // zelfde, maar dan met hoofdletter
      new []{ "\\bout", "âht" }, // 'outdoor'
      new []{ "\\bOut", "Âht" }, // 'Outdoor'
      new []{ "\\b(V|v)er\\b", "$1eâh"}, // 'ver'
      new []{ "ederta", "edâhta"},// 'moedertaal'
      new []{ "dert\\b", "dâht"}, // 'verandert'
      new []{ "(D|d)ert(?![u])","$1eâht"}, // 'dertig', geen 'ondertussen', 'moedertaal'
